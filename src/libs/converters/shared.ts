...
export const doConvertTLSTransport = convertTLSTransport.implement(proxy => {
  const tls: z.infer<typeof SingboxOutboundCommonTlsTransport> = { enabled: true }
...
export const doConvertVmessOrVLESSTransport = convertVmessOrVLESSTransport.implement(proxy => {
  if (proxy['http-opts'] !== undefined) {
    const transport: z.infer<typeof SingboxOutboundCommonVmessOrVLESSTransport> = {
      type: 'http',
    }
    ...
    return transport
  } else if (proxy['ws-opts'] !== undefined && proxy['ws-opts']['v2ray-http-upgrade'] === true) {
    const transport: z.infer<typeof SingboxOutboundCommonVmessOrVLESSTransport> = {
      type: 'httpupgrade',
    }
