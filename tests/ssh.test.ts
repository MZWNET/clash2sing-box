...
describe('doConvertSSH', () => {
  ...
  it('maps private-key to private_key', () => {
    const result = doConvertSSH(makeSSHProxy({ 'private-key': '-----BEGIN OPENSSH PRIVATE KEY-----' }))
    expect(result.private_key).toBe('-----BEGIN OPENSSH PRIVATE KEY-----')
  })

  it('maps private-key-passphrase to private_key_passphrase', () => {
    const result = doConvertSSH(makeSSHProxy({ 'private-key-passphrase': 'my-passphrase' }))
  ...
  it('maps host-key-algorithms to host_key_algorithms', () => {
    ...
    const result = doConvertSSH(makeSSHProxy({ 'host-key-algorithms': algorithms }))
    expect(result.host_key_algorithms).toEqual(algorithms)
  })

  it('maps interface-name to bind_interface', () => {
    const result = doConvertSSH(makeSSHProxy({ 'interface-name': 'eth0' }))
  ...
  it('maps dialer-proxy to detour', () => {
    const result = doConvertSSH(makeSSHProxy({ 'dialer-proxy': 'proxy-chain' }))
