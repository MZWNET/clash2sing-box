import type { ClashProxySSH } from '../src/index.ts'
import type { z } from 'zod'
import { SSHPipeline } from '../src/converters/ssh.ts'
...
describe('ssh pipeline', () => {
  it('converts basic fields: username → user, name → tag', () => {
    const result = SSHPipeline.parse(makeSSHProxy())
  ...
  it('includes optional password', () => {
    const result = SSHPipeline.parse(makeSSHProxy({ password: 'secret' }))
  ...
  it('maps private-key to private_key', () => {
    const result = SSHPipeline.parse(makeSSHProxy({ 'private-key': '-----BEGIN OPENSSH PRIVATE KEY-----' }))
    expect(result.private_key).toBe('-----BEGIN OPENSSH PRIVATE KEY-----')
  })

  it('maps private-key-passphrase to private_key_passphrase', () => {
    const result = SSHPipeline.parse(makeSSHProxy({ 'private-key-passphrase': 'my-passphrase' }))
    expect(result.private_key_passphrase).toBe('my-passphrase')
  })

  it('maps host-key to host_key', () => {
    const hostKeys = ['ssh-rsa AAAA...']
    const result = SSHPipeline.parse(makeSSHProxy({ 'host-key': hostKeys }))
  ...
  it('maps host-key-algorithms to host_key_algorithms', () => {
    ...
    const result = SSHPipeline.parse(makeSSHProxy({ 'host-key-algorithms': algorithms }))
    expect(result.host_key_algorithms).toEqual(algorithms)
  })

  it('maps interface-name to bind_interface', () => {
    const result = SSHPipeline.parse(makeSSHProxy({ 'interface-name': 'eth0' }))
    expect(result.bind_interface).toBe('eth0')
  })

  it('maps routing-mark to routing_mark', () => {
    const result = SSHPipeline.parse(makeSSHProxy({ 'routing-mark': 100 }))
    expect(result.routing_mark).toBe(100)
  })

  it('maps tfo to tcp_fast_open', () => {
    const result = SSHPipeline.parse(makeSSHProxy({ tfo: true }))
    expect(result.tcp_fast_open).toBe(true)
  })

  it('maps mptcp to tcp_multi_path', () => {
    const result = SSHPipeline.parse(makeSSHProxy({ mptcp: true }))
    expect(result.tcp_multi_path).toBe(true)
  })

  it('maps dialer-proxy to detour', () => {
    const result = SSHPipeline.parse(makeSSHProxy({ 'dialer-proxy': 'proxy-chain' }))
