...
function makeVmessOrVLESSProxy(
  overrides: Partial<VmessOrVLESSProxy> = {},
): VmessOrVLESSProxy {
  return {
    name: 'test',
    server: 'example.com',
...
describe('doConvertTLSTransport', () => {
  ...
  it('maps servername to server_name', () => {
    const result = doConvertTLSTransport(
      makeTLSProxy({ servername: 'custom.example.com' }),
    )
  ...
  it('maps sni to server_name', () => {
    const result = doConvertTLSTransport(
      makeTLSProxy({ sni: 'sni.example.com' }),
    )
  ...
  it('maps skip-cert-verify: true to insecure: true', () => {
    const result = doConvertTLSTransport(
      makeTLSProxy({ 'skip-cert-verify': true }),
    )
  ...
  it('does not set insecure when skip-cert-verify is false', () => {
    const result = doConvertTLSTransport(
      makeTLSProxy({ 'skip-cert-verify': false }),
    )
    expect(result).toEqual({ enabled: true })
  ...
  it('maps x-clash2singbox-certificate to certificate array', () => {
    ...
    const result = doConvertTLSTransport(
      makeTLSProxy({ 'x-clash2singbox-certificate': certs }),
    )
