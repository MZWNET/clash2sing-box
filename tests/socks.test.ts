...
describe('doConvertSocks5ToSocks', () => {
  ...
  it('includes username when provided', () => {
    const result = doConvertSocks5ToSocks(
      makeSocksProxy({ username: 'user1' }),
    )
  ...
  it('includes username and password when both provided', () => {
    const result = doConvertSocks5ToSocks(
      makeSocksProxy({ username: 'user1', password: 'pass1' }),
    )
    expect(result.username).toBe('user1')
  ...
  it('throws error for tls=true', () => {
    expect(() =>
      doConvertSocks5ToSocks(makeSocksProxy({ tls: true })),
    ).toThrow('Unsupported layer tls')
  })
})
